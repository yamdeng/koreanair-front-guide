...
export const EventJSX = (params) => {
  const { onSelectFieldEvent } = params;
    ...
    <div className="tw-bg-slate-200 tw-rounded-2xl tw-p-6 tw-mb-4 tw-h-full">
      <div className="tw-p-6 tw-bg-white tw-rounded-2xl tw-h-full tw-h-full tw-overflow-y-scroll">
        ...
        <div className="tw-p-3">
          ...
          <textarea id="name" name="name" className="tw-h-80 av-input-1 tw-rounded-xl" />
        </div>
...
export const AttachmentJSX = (params) => {
  const { fileInputRef, handleFileChange, getFormArray, removeFormFile, saveReport, reports, attachments } = params;
      ...
      <div className="tw-p-6 tw-bg-white tw-rounded-2xl tw-h-full tw-h-full tw-overflow-y-scroll">
        <div className="tw-flex tw-flex-col tw-h-full tw-justify-end">
          ...
          <div className="tw-grow-0">
            ...
            <button onClick={saveReport} className="av-input-1 tw-rounded-lg tw-hidden" key={uuidv4()}>
              Save
            </button>
            ...
            <div className="tw-my-4">
              {(() => {
                return reports.map((element, index) => {
                  return (
                    <div key={index}>
                      id: {element.id}, category: {element.category}, report_id: {element.report_id}, attachment:{' '}
                      {element.attachment}
                    </div>
                  );
                });
