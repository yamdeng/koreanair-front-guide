...
export const SheetSettingFilter = (params) => {
  const { confirmEvent, onClickSelectReportCategory, onClickSelectReportStatus } = params;

  const { filterStruct, updateSubject } = useStore(MyReportListScreenViewModel, (state) => state) as any;
    ...
    <div key={uuidv4()} className="tw-bg-slate-300 tw-rounded-t-2xl tw-p-6">
      ...
      <div className="tw-gap-4 tw-flex tw-flex-col xl:tw-flex-row">
        <div className="tw-bg-slate-200 tw-rounded-2xl tw-p-6 tw-grow">
          ...
          <ReportSelectType1
            placeholder={filterStruct.reportCategory === '' ? 'ALL' : filterStruct.reportCategory}
            onClick={() => {
        ...
        <div className="tw-bg-slate-200 tw-rounded-2xl tw-p-6 tw-grow">
          ...
          <ReportSelectType1
            placeholder={filterStruct.reportStatus === '' ? 'ALL' : filterStruct.reportStatus}
            onClick={() => {
...
export const SheetAddReport = (params) => {
  ...
  return (
    <div className="tw-bg-slate-300 tw-rounded-t-2xl tw-p-6" key={uuidv4()}>
      {/* 보고서 종류 */}
      <div className="tw-w-full tw-pb-6 tw-text-3xl">보고서 종류</div>
...
export const SheetSelectReportStatus = (params) => {
  ...
  return (
    <div className="tw-bg-slate-300 tw-rounded-t-2xl tw-p-6">
      {/* 보고서 종류 */}
      <div className="tw-w-full tw-pb-6 tw-text-3xl">보고서 상태</div>
      <div className="tw-flex tw-gap-4 tw-flex-col">
        {(() => {
