...
export const MyReportListScreenViewModel = create<any>((set, get) => ({
  ...
  goWriteReport: () => {
    ...
    const reportInfo = ReportConfig.filter((element) => {
      return element.category === 'csr';
    })[0];
  ...
  // 검색 필터 BottomSheet
  addSheetSettingFilter: () => {
    const { addSheet, onClose, addSheetSelectReportCategory, addSheetSelectReportStatus, confirmFilter } = get();

    addSheet({
      name: 'SettingFilter',
      isShow: false,
      jsx: () => {
        const { filterStruct } = get();
  ...
  addSheetSelectReportStatus: (completion) => {
    ...
    addSheet({
      ...
      isShow: false,
      jsx: () => {
        return <SheetSelectReportStatus status={list} />;
      },
      onClose: onClose,
    });
  },
