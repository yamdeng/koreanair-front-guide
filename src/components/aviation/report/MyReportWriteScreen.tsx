...
import { BottomSheetLayoutWhenWrite } from './forms/BottomSheet';
...
export default function MyReportWriteScreen(params) {
  const { init, currentFragment, cardFragmentClipEvent, onCancel, onNext } = useStore(
  ...
  const baseTabStyleOn = `tw-px-3 tw-bg-slate-200 tw-py-3 tw-mx-2 tw-rounded-t-xl`;
            ...
            switch (currentFragment.tab) {
              case 'Flight': {
                switch (currentFragment.sub) {
                  case 'Search':
                    return <FlightSearchJSX />;
                  case 'Detail':
                    return <FlightDetailJSX />;
                }
                break;
              }
              case 'Event': {
                return <EventJSX onSelectFieldEvent={() => {}} />;
              }
              case 'Attachment': {
                return (
                  <AttachmentJSX
        ...
        {(() => {
          const html = (
            <div key={uuidv4()} className="tw-bg-slate-300 tw-rounded-t-2xl tw-p-6">
              {/* 보고서 종류 */}
              <div className="tw-w-full tw-pb-6 tw-text-3xl">Event</div>
          ...
          return BottomSheetLayoutWhenWrite({
            isShow: true,
            jsx: () => {
              return html;
            },
